...
class PW3DDataset(BaseDataset):
    ...
    def __init__(self, cfg, estimator='spin', return_type='3D', phase='train'):
        ...
        self.ground_truth_path = cfg.DATASET.PW3D.GROUND_TRUTH_PATH
        self.detected_path = os.path.join(cfg.DATASET.PW3D.DETECTED_PATH,
                                          self.estimator)
        ...
        if self.return_type == '3D':
            self.input_dimension = cfg.DATASET.PW3D.KEYPOINT_NUM * 3
            self.keypoint_root = cfg.DATASET.PW3D.KEYPOINT_ROOT
